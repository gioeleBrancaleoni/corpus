...
describe("recommendModels — tier selection", () => {
  const cases: [vram: number, chat: string, embed: string][] = [
    [32, "qwen3.6:27b", "snowflake-arctic-embed2:latest"],
    [24, "qwen3.6:27b", "snowflake-arctic-embed2:latest"],
    [23.9, "gpt-oss:20b", "snowflake-arctic-embed2:latest"],
    [16, "gpt-oss:20b", "snowflake-arctic-embed2:latest"],
    [12, "qwen2.5:7b", "nomic-embed-text:latest"],
    [8, "qwen2.5:7b", "nomic-embed-text:latest"],
    [4, "llama3.2:3b", "nomic-embed-text:latest"],
    [0, "llama3.2:3b", "nomic-embed-text:latest"],
  ];
...
describe("recommendModels — installed vs must-pull", () => {
  it("marks missing models with the exact pull command", () => {
    ...
    expect(r.embed.pullCommand).toBe("ollama pull snowflake-arctic-embed2:latest");
  });
  ...
  it("marks the tier model installed when present (no pull command)", () => {
    ...
    expect(r.embed).toEqual({ model: "snowflake-arctic-embed2:latest", installed: true });
  });
  ...
  it("prefers an installed model from a lighter tier over pulling the tier model", () => {
    ...
    expect(r.embed).toEqual({ model: "nomic-embed-text:latest", installed: true });
  });
